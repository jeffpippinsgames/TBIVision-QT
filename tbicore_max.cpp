...
void Max::recieveNewCVMat(const Mat &_mat)
{
    ...
    cv::Mat _inliers_mat = cv::Mat::zeros(_raw_mat.rows, _raw_mat.cols, CV_8UC3);
    ...
    if(m_gausian_decluster_ds->buildGausianClusterDataSet(_threshold_mat, m_gausian_decluster_parameters) == TBIDataSetReturnType::Ok)
    {
        ...
        if(m_gausian_decluster_ds->extractDistributionSet(*m_gausian_decluster_distro) == TBIDataSetReturnType::Ok)
        {
            ...
            if(_breakindex != -1)
            {
                ...
                _tmpransacparm.makeEqual(m_left_bwl_ransac);
                _tmpransacparm.setDistanceThreshold(5);
                _tmpransacparm.setIterations(50);
                TBIRansac::doRansac(m_ransac_left_bwl, _tmpransacparm, *m_joint_ds);
                if(m_ransac_left_bwl.isValid())
                {
                    ...
                    //Build Inliers Set
                    if(m_gausian_decluster_ds->extractDataSetForInliers(*m_left_ransac1_bwl_ds, m_ransac_left_bwl, 5.0, 0, _breakindex) == TBIDataSetReturnType::Ok)
                    {
                        TBIRansac::doRansac(m_ransac_left_bwl, m_left_bwl_ransac, *m_left_ransac1_bwl_ds);
                        if(m_ransac_left_bwl.isValid())
                        {
                            m_ransac_left_bwl.remakeLine(0, _raw_mat.cols);
                            m_ransac_left_bwl.drawOnMat(_ransac_mat, CV_RGB(75,75,255));
                ...
                //Right BWL
                TBIRansac::doRansac(m_ransac_right_bwl, m_right_bwl_ransac, *m_joint_ds);
                if(m_ransac_right_tsl.isValid())
                {
