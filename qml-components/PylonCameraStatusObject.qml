...
Item
{
    ...
    property bool cameraconnected: PylonCameraParameters.connected
    property string cameraname: PylonCameraParameters.cameraName

    property int rect_x: 0
    property int rect_y: 0
    property int rect_width: rootrectId.width
    property int rect_height: rootrectId.height
    ...
    function setStatus()
    {
        if(rootitemId.cameraconnected)
        {
            rootrectId.border.color = rootitemId.mode4color;
            nametextId.text = rootitemId.cameraname;
            nametextId.color = "chartreuse";
            glowId.color = "chartreuse";
        }
        else
        {
            rootrectId.border.color = rootitemId.mode2color;
            nametextId.text = "No Camera Connected";
            nametextId.color = "red";
            glowId.color = "red";
        }
    }
    ...
    Rectangle
    {
        ...
        Image
        {
            ...
            height: 55;
            x: 5
            y: (rootrectId.height - cameraimageId.height)/2
            visible: true
        ...
        {
            id: glowId
            anchors.fill: cameraimageId
            radius: 8
            samples: 25
            color: "yellow"
            source: cameraimageId
            opacity: .5
        }
        ...
        Text
        {
            ...
            text: rootitemId.cameraname
            font.pointSize: 20
            width: nametextId.implicitWidth
            height: nametextId.implicitHeight
            x: cameraimageId.x+cameraimageId.width+10
            y: (rootrectId.height - nametextId.height)/2
            color: rootitemId.textcolor
            opacity: .7

            onTextChanged:
            {
                if(nametextId.text.length > 20)
                {
                    nametextId.text = nametextId.text.substring(0,19);
                }
            }
        }
